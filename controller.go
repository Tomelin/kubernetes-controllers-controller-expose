...
func (c *controller) processItem() bool {
	...
	if apierrors.IsNotFound(err) {
		...
		if err != nil {
			fmt.Printf("deleting pod %s, error %s\n", name, err.Error())
			return false
		}
...
func (c *controller) syncDeployment(ns, name string) error {
	...
	// get address from pod
	pods, err := c.clientset.CoreV1().Pods("").List(ctx, metav1.ListOptions{FieldSelector: "status.phase=Pending"})
	if err != nil {
		fmt.Printf("getting pods erros is %s\n", err.Error())
		return err
	}
	fmt.Println(pods)
